...
# Now, we need a model. As we don't solve anything in this tutorial, we don't need to add the solver.
new_model = Model()
...
## Exercise 1.1: Create continuous, integer, and binary variables
# Create three different variables of different types without any additional bound: 
# 'continuous_var' as a continuous variable for 'new_model', 
# 'integer_var' as an integer variable for 'new_model',
# and 'binary_var' as a binary variable for 'new_model'.
...
# If we need to define larger models, creating each variable manually can become impossible.
# JuMP supports creating variables in containers like arrays, sets, dictionaries, and matrices.
# For example:
# @variable(modelName, variableName[1:20], Bin)
# This would create a container with 20 variables. 
# To create a set based on a range, we could do:
# new_range = 1:100
...
# This would create a container with a matrix of continuous variables without any bound.
# Note that you will have to change 'modelName' and 'variableName' according to your instance.

## Exercise 2.1: Create an array of variables
...
## Exercise 2.3: Create indexed variables based on a set
# Create variables 'W' indexed by set N of size 1, 2, ..., 6 with non-negative lower bounds.
...
println("You've learned how to create variables in containers, manage different types of 
    variables, and work with indexed variables.")
