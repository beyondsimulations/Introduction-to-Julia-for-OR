...
# You'll learn about defining and managing objective functions and constraints within containers (arrays, dictionaries, matrices), 
# and how to implement conditional constraints effectively.
...
# Again, we need a model. As we don't solve anything in this tutorial, we don't need to add the solver.
...
# First, we need a container with variables for the objective function. For example:
...
# Exercise 45.1: Define an array of variables and an objective function for 'another_model'.
# The variables should be called 'profit' and have a range from 1:10. It has an upper bound of 12.
# The objective should be a Maximization of the sum of all 'profits'.
...
# This is especially important when models become larger! To define a constraint within 
# a container, we can do, for example, the following:
# @constraint(modelName, constraintName[i in 1:3], variableName[i] <= 100)
# This would create a constraint called constraintName for each i - thus 1,2, and 3 - where variableName[1],
# variableName[2], and variableName[3] are restricted to be maximally 100.

# Exercise 2.1: Define constraints called 'maxProfit' using an array of variables.
# The Constraint: Each profit defined in the previous task should be less than or equal to 12.
...
# allowing for dynamic and flexible model formulations.
# To define a constraint within  a container under conditions, we can do the following:
# @constraint(modelName, constraintName[i in 1:3; i <= 2], variableName[i] <= 50)
# This would create a constraint called constraintName for each i - thus 1,2, and 3 - where variableName[1],
# variableName[2] are restricted to be maximally 50 and variableName[3] was not restricted.

# Exercise 47.1: Add a conditional constraint 'smallProfit' to the previous model.
...
println("You've learned how to define objective functions and constraints using container variables.")
